...
export const quote = (status, muted, quoteMuted, handleExpandedQuoteToggle, identity, media, withAction, contextType = 'home') => {
  ...
  const quoteInner = (() => {
    ...
    if (quoteMuted || hideUnlisted) {
      ...
      return (
        <div className={classNames('status__content', { 'muted-quote': quoteMuted, 'unlisted-quote': hideUnlisted, 'status__content--with-action': withAction })}>
          {content}
        </div>
      );
    }
    ...
    return (
      <div>
        ...
        <Link to={`/@${status.getIn(['account', 'acct'])}/${status.get('id')}`}>
          <StatusContent status={quoteStatus} expanded={!status.get('quote_hidden')} onExpandedToggle={handleExpandedQuoteToggle} quote />
        </Link>
...
class Status extends ImmutablePureComponent {
  ...
  render () {
    const { intl, hidden, featured, unfocusable, unread, showThread, scrollKey, pictureInPicture, previousId, nextInReplyToId, rootId, skipPrepend, avatarSize = 46, quoteMuted, contextType } = this.props;
    ...
    const statusAvatar = (status, account) => {
      if (account === undefined || account === null) {
        return <Avatar account={status.get('account')} size={avatarSize} />;
      } else {
    ...
    const identity = (status, account) => (
      <a onClick={this.handleAccountClick} data-acct={status.getIn(['account', 'acct'])} href={`/@${status.getIn(['account', 'acct'])}`} title={status.getIn(['account', 'acct'])} data-hover-card-account={status.getIn(['account', 'id'])} className='status__display-name' target='_blank' rel='noopener noreferrer'>
          ...
          <div className={classNames('status', `status-${status.get('visibility')}`, { 'status-reply': !!status.get('in_reply_to_id'), 'status--in-thread': !!rootId, 'status--first-in-thread': previousId && (!connectUp || connectToRoot), muted: this.props.muted })} data-id={status.get('id')}>
            ...
            {expanded && (
              <>
                ...
                {media(status)}
                {quote(status, this.props.muted, quoteMuted, this.handleExpandedQuoteToggle, identity, media, this.context.router, contextType)}
                {hashtagBar}
              </>
            )}
