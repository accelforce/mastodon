...
export const DetailedStatus: React.FC<{
  ...
  onToggleQuoteHidden?: (status: any) => void;
  ...
  const quoteMuted = useAppSelector((state) => {
    if (!status.get('quote')) {
      return !!status.get('quote_id', null);
    }

    const id = status.getIn(['quote', 'account', 'id']);
    return (
      id !== null &&
      (state.relationships.get(id)?.muting ||
        state.relationships.get(id)?.blocking ||
        state.relationships.get(id)?.blocked_by ||
        state.relationships.get(id)?.domain_blocking ||
        status.getIn(['quote', 'muted']))
    );
  });
  ...
  const handleExpandedQuoteToggle = useCallback(() => {
    if (onToggleQuoteHidden) onToggleQuoteHidden(status);
  }, [onToggleQuoteHidden, status]);
  ...
  const media = (status: any, quote: boolean) => {
    if (pictureInPicture.get('inUse')) {
      return (
        <PictureInPicturePlaceholder aspectRatio={attachmentAspectRatio} />
      );
